            ...
            resp.on('end', () => {
                if (user.id === message.author.id) {
                    const msg = new Discord.MessageEmbed()
                        .setColor('#9a3deb')
                        .setTitle(`${message.author.username}, you have snuggled yourself!`)
                    ...
                    const msg = new Discord.MessageEmbed()
                        .setColor('#9a3deb')
                        .setTitle(`${message.author.username} snuggled me!`)
                        .setImage(JSON.parse(data).results[Math.floor(Math.random() * Object.keys(JSON.parse(data).results).length)].media[0].gif.url);
                    return message.channel.send(msg);
                }
                ...
                const msg = new Discord.MessageEmbed()
                    .setColor('#9a3deb')
                    .setTitle(`${user.username}, you have been snuggled by ${message.author.username}!`)
                    .setImage(JSON.parse(data).results[Math.floor(Math.random() * Object.keys(JSON.parse(data).results).length)].media[0].gif.url);
